...
Перед запуском приложения (особенно через `docker-compose`) необходимо настроить переменные окружения.

1.  Создайте файл `.env` в корневой директории проекта.
2.  Заполните его необходимыми значениями. Как минимум, вам нужно указать ваш токен для валидации данных с MAX Bridge:

    ```env
    # .env
    DATABASE_URL=postgresql://postgres:postgres@db:5432/postgres
    BOT_TOKEN=your_max_bridge_bot_token_here
    ```

    Замените `your_max_bridge_bot_token_here` на реальный токен вашего бота.
