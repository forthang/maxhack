  ...
  const handleBuy = async (item: Item) => {
    ...
    try {
      const resp = await fetch('/api/profile/store/purchase', {
        ...
        body: JSON.stringify({ user_id: currentUser.id, item_id: item.name, cost: item.cost }),
      });
            ...
            >
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 bg-gray-100 dark:bg-gray-700 rounded-md flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-gray-500 dark:text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-1.3 5M17 13l1.3 5M6 21h12" />
                  </svg>
                </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <button
                  ...
                  disabled={purchased.includes(item.name) || !currentUser}
