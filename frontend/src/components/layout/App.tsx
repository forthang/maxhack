...
const App: React.FC = () => {
  ...
  const { userId, isValidating, isValidated, validationError } = useMaxApp();
  ...
  const NavLink: React.FC<{ to: string; label: string; icon: React.ReactNode }> = ({ to, label, icon }) => {
    ...
    return (
      <Link
        to={to}
        className="flex flex-col items-center justify-center w-1/4 h-full text-sm transition-colors group"
      >
        <div
          className={`p-2 rounded-full transition-all ${isActive ? 'bg-brand/20 text-brand' : 'text-neutral-500 group-hover:bg-neutral-200 dark:group-hover:bg-neutral-700'}`}
        >
          {icon}
        </div>
